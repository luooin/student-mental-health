<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="健康状态" prop="jiankangzhuangtai">
        ...
        <div v-else>
          <el-form-item class="input" label="健康状态" prop="jiankangzhuangtai">
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="心里疾病" prop="xinlijibing">
          <el-select :disabled="ro.xinlijibing" v-model="ruleForm.xinlijibing" placeholder="请选择心里疾病">
            <el-option
                v-for="(item,index) in xinlijibingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="心里疾病" prop="xinlijibing">
	      <el-input v-model="ruleForm.xinlijibing"
                placeholder="心里疾病" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="健康评价" prop="jiankangpingjia">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.jiankangpingjia" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.jiankangpingjia" label="健康评价" prop="jiankangpingjia">
                    <span v-html="ruleForm.jiankangpingjia"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.jiankangzhuangtai = true;
          ...
          if(o=='xinlijibing'){
            this.ruleForm.xinlijibing = obj[o];
	    this.ro.xinlijibing = true;
            continue;
          }
          if(o=='jiankangpingjia'){
            this.ruleForm.jiankangpingjia = obj[o];
	    this.ro.jiankangpingjia = true;
            continue;
          }
          if(o=='dengjishijian'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.jiankangzhuangtaiOptions = "优,良,一般".split(',')
            this.xinlijibingOptions = "有,无".split(',')
    },
    // 下二随
    xuehaoChange () {
      this.$http({
    ...
    info(id) {
      this.$http({
        url: `xueshengxinlijiankang/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.jiankangpingjia = this.ruleForm.jiankangpingjia.replace(reg,'../../../springboot9701n/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xueshengxinlijiankang/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xueshengxinlijiankangCrossAddOrUpdateFlag = false;
						 this.parent.search();
